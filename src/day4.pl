...
anagram(X, XS) :-
  maplist(atom_codes, XS, AS), atom_codes(X, A),  member(AA, AS),
  forall(member(A1, A), member(A1, AA)), forall(member(A2, AA), member(A2, A)).

no_anagrams(XS) :- forall(member(X, XS), (select(X, XS, YS), not(anagram(X, YS)))).

filter2([], []).
filter2([H|T], R) :-
  no_anagrams(H) -> filter2(T, R1), R = [H|R1] ;  filter2(T, R).
