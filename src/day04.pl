...
anagram(X, XS) :-
  ...
  forall(member(A1, A), member(A1, A)).
...
% Day 4.A solution
day04a(X) :-
  from_file("Inputs/day04.txt", F), include(is_set, F, Fi), length(Fi, X).

% Day 4.B solution
day04b(X) :-
  from_file("Inputs/day04.txt", F), include(no_anagrams, F, Fi), length(Fi, X).
